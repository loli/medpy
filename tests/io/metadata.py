...
####
                ...
                for dtype in __dtypes:
                    ...
                    for suffix_from in __suffixes:
                        ...
                        try:
                            ...
                            logger.debug('Could not set the header meta-data for image of type {} with shape={}/dtype={}. This should not happen and hints to a bug in the code. Signaled reason is: {}'.format(suffix_from, arr_save.shape, dtype, e))
                            unsupported_types[suffix_from][suffix_from][ndim][dtype] = e.message                            
                            continue
    ...
    def __same_seq(self, seq1, seq2):
        ...
        for e1, e2 in zip(seq1, seq2):
            diff = abs(e1 - e2)
            if diff > self.__delta: return False
        return True
