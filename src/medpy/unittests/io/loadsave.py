...
class TestIOFacilities(unittest.TestCase):
    ...
    __important = ['.nii', '.nii.gz', '.hdr', '.img', '.img.gz', '.dcm', '.dicom', '.mhd', '.nrrd']
    ...
    # list of image formats ITK is theoreticvally able to load
    ...
    def test_SaveLoad(self):
        ...
        __ndims = [1, 2, 3, 4, 5]
        __dtypes = [scipy.int8, scipy.int16, scipy.int32, scipy.int64,
                ...
                for dtype in __dtypes:
                    ...
                    for suffix in __suffixes:
                        ...
                        try:
                            ...
                            # check if the old and new array are the same
        ...
        if inconsistent:
            print '\nthe following configurations show inconsistent saving and loading behviour:'
            print 'type\tndim\tdtype\t\terror'
    ...
    def __is_lossless(self, _from, _to):
        ...
        if _from in __int_order:
            if _to in __int_order[__int_order.index(_from):]: return True
            else: return False
        elif _from in __float_order:
            if _to in __float_order[__float_order.index(_from):]: return True
            else: return False
        elif _from in __uint_order:
            if _to in __uint_order[__uint_order.index(_from):]: return True
            else: return False
        else: return False
