...
"""A metrics class computing surface metrics over two 3D-images contain each a binary object."""
...
__version__ = "r0.3, 2011-12-01"
__email__ = "oskar.maier@googlemail.com"
__status__ = "Release"
__description__ = "Surface metrics class."
...
class Surface(object):
    ...
    def __init__(self, mask, reference, physical_voxel_spacing = [1,1,1], mask_offset = [0,0,0], reference_offset = [0,0,0]):
        ...
        """
        # compute edge images
        mask_edge_image = Surface.ComputeContour(mask)
        reference_edge_image = Surface.ComputeContour(reference)
        ...
        # set member vars
        self._mask_edge_points = mask_edge_points
        self._reference_edge_points = reference_edge_points
        
    def GetMaximumSymmetricSurfaceDistance(self):
        """
        ...
        A_B_distance = self.GetMaskReferenceNn().max()
        B_A_distance = self.GetReferenceMaskNn().max()
        
        # compute result and return
        return max(A_B_distance, B_A_distance)
        
    def GetRootMeanSquareSymmetricSurfaceDistance(self):
        """
        ...
        """
        # get object sizes
        mask_surface_size = len(self.GetMaskEdgePoints())
        reference_surface_sice = len(self.GetReferenceEdgePoints())
        
        # get minimal nearest neighbours distances
        A_B_distances = self.GetMaskReferenceNn()
        B_A_distances = self.GetReferenceMaskNn()
    ...
    def GetAverageSymmetricSurfaceDistance(self):
        """
    ...
    def GetMaskReferenceNn(self):
        """
        @return: The distances of the nearest neighbours of all mask edge points to all
                 reference edge points.
        """
        # Note: see note for @see GetReferenceMaskNn
        if None == self._mask_reference_nn:
            tree = scipy.spatial.KDTree(self.GetMaskEdgePoints())
            self._mask_reference_nn, _ = tree.query(self.GetReferenceEdgePoints())
        return self._mask_reference_nn
            
    def GetReferenceMaskNn(self):
        """
        ...
        if None == self._reference_mask_nn:
            tree = scipy.spatial.KDTree(self.GetReferenceEdgePoints())
            self._reference_mask_nn, _ = tree.query(self.GetMaskEdgePoints())
        return self._reference_mask_nn
        
    def GetMaskEdgePoints(self):
        """
        @return: The edge points of the mask object.
        """
        return self._mask_edge_points

    def GetReferenceEdgePoints(self):
        """
        @return: The edge points of the reference object.
        """
        return self._reference_edge_points              
    
    @staticmethod
    def ComputeContour(array):
        """
