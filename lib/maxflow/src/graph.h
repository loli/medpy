...
public:
	...
	// Added by Oskar Maier
	// Behaves similar to add_edge, but avoids creating new arcs when called repeatedly.
	// Instead retrieved the existing arc and adds the new cap and rev_cap to the
	// already existing ones.
	void sum_edge(node_id _i, node_id _j, captype cap, captype rev_cap);

	// Added by Oskar Maier
	// Returns the weight for the arc i->j. If no such arc exists, returns 0.
	// If multiple arcs exists between the two nodes (as possible when calling add_edge
	// multiple time), the cap of the first encountered is returned.
	captype get_edge(node_id i, node_id j);
...
public:
	...
	// Added by Oskar Maier
	// Returns the pointer to the arc i->j if exsiting. Else returns NULL.
	arc_id get_arc(node *i, node *j);
...
template <typename captype, typename tcaptype, typename flowtype> // Added by Oskar Maier
	inline void Graph<captype,tcaptype,flowtype>::sum_edge(node_id _i, node_id _j, captype cap, captype rev_cap)
{
	assert(_i >= 0 && _i < node_num);
	assert(_j >= 0 && _j < node_num);
	assert(_i != _j);
	...
	arc *a = get_arc(i, j); // retrieve arc between the nodes

	if (a) // arc already exists
	{
		arc *a_rev = a->sister;
		a -> r_cap += cap;
		a_rev -> r_cap += rev_cap;
	} else { // create new arc
		add_edge(_i, _j, cap, rev_cap);
	}
}

template <typename captype, typename tcaptype, typename flowtype> // Added by Oskar Maier
	inline captype Graph<captype,tcaptype,flowtype>::get_edge(node_id _i, node_id _j)
{
	assert(_i >= 0 && _i < node_num);
	assert(_j >= 0 && _j < node_num);
	assert(_i != _j);
	...
	if (a) return a -> r_cap;
	else return (captype) 0;
}

template <typename captype, typename tcaptype, typename flowtype> // Added by Oskar Maier
	inline typename Graph<captype,tcaptype,flowtype>::arc* Graph<captype,tcaptype,flowtype>::get_arc(node *i, node *j)
{
	arc* a = i->first;
	while (a)
	{
