...
def main():
    ...
    def morph(arr, values):
        ...
        if not 0 == select_value(values, args.order[0]):
            arr = functions[args.order[0]](arr, footprint, select_value(values, args.order[0]))
        if not 0 == select_value(values, args.order[1]):
            arr = functions[args.order[1]](arr, footprint, select_value(values, args.order[1]))
        if not 0 == select_value(values, args.order[2]):
            arr = functions[args.order[2]](arr, footprint, select_value(values, args.order[2]))
...
def getParser():
    ...
    parser.add_argument('--order', choices=['ced', 'cde', 'ecd', 'edc', 'dce', 'dec'])
    parser.add_argument('--startc', type=int)
    parser.add_argument('--starte', type=int)
    parser.add_argument('--startd', type=int)
    parser.add_argument('--endc', type=int)
    parser.add_argument('--ende', type=int)
    parser.add_argument('--endd', type=int)
